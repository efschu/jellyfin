...
type VsFilterPreset = 'anime-upscaled' | 'anime-interpolated' | 'upscale-only' | 'custom';
...
export const VsFilterSettings: React.FC<VsFilterSettingsProps> = ({ onSave }) => {
    ...
    const updateSetting = useCallback(
        <K extends keyof EncodingOptions>(key: K, value: EncodingOptions[K]) => {
        ...
        try {
            const response = await fetch('/EncodingOptions/VsFilter', {
                ...
                headers: {
                    ...
                    'X-Emby-Token':
                        (window as unknown as { ApiClient?: { accessToken(): string } })
    ...
    const showInterpolation =
        settings.VsFilterPreset === 'anime-interpolated' ||
        settings.VsFilterPreset === 'upscale-only';
    ...
    return (
        <Paper sx={{ p: 3 }}>
            ...
            <Typography variant='body2' color='text.secondary' sx={{ mb: 3 }}>
                Enable AI-powered upscaling and frame interpolation using FFmpeg&apos;s VapourSynth demuxer
                with dynamically generated scripts. The pipeline writes a .vpy script that loads the source
                video, applies AI processing, and outputs the result.
            </Typography>
            ...
            {isCustomPreset && (
                <Card sx={{ mt: 3 }}>
                    <CardContent>
                        <Typography variant='h6' gutterBottom>
                            Custom VapourSynth Script Path
                        </Typography>
                        <Typography variant='body2' color='text.secondary' sx={{ mb: 2 }}>
                            Path to a custom VapourSynth script (.vpy) that handles the source loading
                            and output. The script receives the source via standard VapourSynth loaders.
                        </Typography>
                        <TextField
                            fullWidth
                            ...
                            disabled={!settings.EnableVsFilterPipeline}
                            placeholder='/config/vapoursynth/custom.vpy'
                        />
                    </CardContent>
                </Card>
            )}
...
export default VsFilterSettings;
